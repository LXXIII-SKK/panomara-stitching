...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Root folder containing scene_* directories, or split folders such as development/test/failure_analysis.",
    )
    parser.add_argument(
        "--verify-images",
        action="store_true",
...
def list_validation_scene_sources(root: Path) -> list[tuple[str, Path]]:
    direct_dirs = list_scene_dirs(root)
    if any((path / "meta.json").exists() for path in direct_dirs):
        return [(root.name, scene_dir) for scene_dir in direct_dirs]

    scene_sources: list[tuple[str, Path]] = []
    for split_dir in direct_dirs:
        for scene_dir in list_scene_dirs(split_dir):
            if (scene_dir / "meta.json").exists():
                scene_sources.append((split_dir.name, scene_dir))
...
def main() -> int:
    ...
    scene_sources = list_validation_scene_sources(root)
    split_counter: Counter[str] = Counter()

    for split_name, scene_dir in scene_sources:
        split_counter.update([split_name])
    ...
    print(f"Scenes checked: {len(scene_sources)}")
    if split_counter:
