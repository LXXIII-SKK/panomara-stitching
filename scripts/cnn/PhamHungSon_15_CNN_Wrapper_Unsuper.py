...
_DEFAULT_CKPT_DIR = os.path.normpath(os.path.join(_CODE_DIR, "PhamHungSon_15_CNN_CheckpointsUnsup5000"))
...
def _checkpoint_path():
    ...
    return os.path.join(_DEFAULT_CKPT_DIR, "19model.ckpt")
...
def MergeImages(Imagelist):
    ...
    for i in range(1,Number):
        ...
        H4pt_batch_j = H4pt_batch_j.to(device).float()
        Ca_batch_j = Ca_batch_j.to(device).float()
        Cb_batch_j = Cb_batch_j.to(device).float()
        Pa_batch_j = Pa_batch_j.to(device).float()
...
def main():
    ...
    if No_of_images == 0:
        ...
        print("Please place your color or grayscale images inside this folder and run again to perform deep unsupervised homography stitching.\n")
        return
