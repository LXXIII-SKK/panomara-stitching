...
DESCRIPTOR_ALIASES = {
    ...
    "BRISK": "BRISK",
    "SURF": "SURF",
    "HARRIS_ORB": "HARRIS_ORB",
    "HARRIS+ORB": "HARRIS_ORB",
    "HARRIS_ORB": "HARRIS_ORB",
...
ALL_DESCRIPTORS = ["SIFT", "ORB", "HARRIS_ORB", "HARRIS_HOG", "AKAZE", "BRISK"]
EXPLICIT_DESCRIPTORS = ALL_DESCRIPTORS + ["SURF"]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  conda run -n image_recognition python scripts/extract_features.py --split test --scene scene_3 --descriptor SIFT\n"
            "  conda run -n image_recognition python scripts/extract_features.py --split development --descriptor ORB --descriptor AKAZE\n"
            "  conda run -n image_recognition python scripts/extract_features.py --descriptor all --overwrite\n\n"
    ...
    parser.add_argument(
        "--split",
        action="append",
        help="Split to process. Repeat for multiple splits. If omitted, all available preprocessing splits are used.",
    )
    parser.add_argument(
        "--scene",
        action="append",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Descriptor/pipeline to run: SIFT, ORB, Harris_ORB, Harris_HOG, AKAZE, BRISK, SURF, or all. "
            "'all' excludes SURF because it is unavailable in this OpenCV build unless explicitly supported. "
...
def normalize_descriptor(raw_value: str) -> str:
    ...
    token = token.replace("HARRIS_ORB", "HARRIS_ORB").replace("HARRIS_HOG", "HARRIS_HOG")
...
def extract_features(gray: np.ndarray, descriptor: str, args: argparse.Namespace) -> tuple[list[cv2.KeyPoint], np.ndarray | None]:
    ...
    if descriptor == "SURF":
        if not (hasattr(cv2, "xfeatures2d") and hasattr(cv2.xfeatures2d, "SURF_create")):
            raise RuntimeError(
                "SURF is unavailable in this OpenCV build. It usually requires opencv-contrib with nonfree support."
            )
        detector = cv2.xfeatures2d.SURF_create(hessianThreshold=400)
        keypoints, descriptors = detector.detectAndCompute(gray, None)
        return sort_and_limit_keypoints(list(keypoints or []), descriptors, max_features)

    if descriptor == "HARRIS_ORB":
        ...
        if not keypoints:
            return [], None
        orb = cv2.ORB_create(nfeatures=max_features, fastThreshold=args.orb_fast_threshold)
